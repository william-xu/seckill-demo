...
var seckill ={
    ...
    countdown: function (seckillId, nowTime, startTime, endTime) {
        ...
        if(nowTime > endTime){
            ...
            startCounterSpan.countdown(killTime,function(event){
                var format = event.strftime('秒杀即将开始: %D天 %H时 %M分 %S秒');
                startCounterSpan.html(format);
                /*时间完成后回调事件*/
            }).on('finish.countdown',function(){            	
            	seckill.startSeckill(seckillId,seckillBox,endTime,startCounterSpan);
            });
        }else{
            //秒杀开始
            seckill.startSeckill(seckillId,seckillBox,endTime,startCounterSpan);
    ...
    startSeckill: function(seckillId, seckillBox, endTime,startCounterSpan){
        ...
        endCounterSpan.countdown(end,function(event){
            var format = event.strftime('距离秒杀结束: %D天 %H时 %M分 %S秒');
            endCounterSpan.html(format);
            /*时间完成后回调事件*/
        }).on('finish.countdown',function(){
        	seckillBox.hide();
        	endCounterSpan.html("秒杀已经结束！");        	
